...
export default function Home() {
	...
	function imageUploaderHandleImageChange(e) {
		...
		const allowOnly = process.env.REACT_APP_IMG_UPLOADER_API_ALLOWED_FILETYPE.split(", ");
		const fileSize = Number(process.env.REACT_APP_IMG_UPLOADER_IMAGE_SIZE_LIMIT) * 1024 * 1024;
			...
			for (let i = 0; i < fileInfo.length; i++) {
				...
				if (allowOnly.includes(files.type)) {
					if (files.size <= fileSize) {
						...
						e.target.value = null;
						toast.error(`Image size must be less than ${process.env.REACT_APP_IMG_UPLOADER_IMAGE_SIZE_LIMIT}MB.`);
						break;
					}
				} else {
					e.target.value = null;
					toast.error(`Image file type must be ${process.env.REACT_APP_IMG_UPLOADER_API_ALLOWED_FILETYPE.replace(/image\//g, "")} only.`);
					break;
				}
			}
	...
	async function imgUploaderHandleSubmit(e) {
		...
		imgUploaderSelectedImages.forEach((image) => {
			formData.append(process.env.REACT_APP_IMG_UPLOADER_IMAGE_INPUT_NAME, image);
		});

		await fetch(process.env.REACT_APP_IMG_UPLOADER_API_PATH, {
			method: "POST",
			body: formData,
		})
			.then((res) => res.json())
	...
	return (
		<main className="p-0 m-0 vh-100 vw-100 d-flex justify-content-center align-items-center">
			<div className="container-fluid row">
				<div className="card m-2 p-2 w-75 col-12 col-md-10 mx-auto rounded border border-2">
					<div className={`img-uploader-main-${process.env.REACT_APP_IMG_UPLOADER_THEME}`}>
						<form className="py-2 row" onSubmit={imgUploaderHandleSubmit}>
							...
							<div className="col-12 mb-3">
								<label className={`text-${process.env.REACT_APP_IMG_UPLOADER_THEME === "light" ? "dark" : "light"}`}>Image Upload</label>
								<div className={`custom-file-upload custom-file-upload-borderBack-${process.env.REACT_APP_IMG_UPLOADER_THEME}`} htmlFor="file">
											...
											{imgUploaderSelectedImages?.map((image, index) => (
												<div className="position-relative col-4 col-xl-1 col-md-2 my-1" key={index + index}>
													...
													<button
														...
														className={`preview-close preview-close-${process.env.REACT_APP_IMG_UPLOADER_THEME}`}
													>
									...
									<label>
										<div className={`add-moreRing add-moreRing-${process.env.REACT_APP_IMG_UPLOADER_THEME}`} htmlFor="file">
											<i className="fas fa-plus fa-sm" />
										</div>
										<input
											type="file"
											name={process.env.REACT_APP_IMG_UPLOADER_IMAGE_INPUT_NAME}
											accept={process.env.REACT_APP_IMG_UPLOADER_API_ALLOWED_FILETYPE}
